...
whenToUse: After shipping any -rN candidate or stable. Always sync both README files (en + ru).
---
...
Use the DSH `grep` tool to locate `build.ps1 -Version` and `build-mac.sh` examples in both README files, then use `edit` for exact replacements. Re-read the four changed lines and confirm both languages use the exact shipped version.
...
- Не редактировать feature list в README на каждый -rN — bloat.
