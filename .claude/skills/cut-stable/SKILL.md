...
when: The latest -rN passed all hard gates and the user explicitly said cut, ok, promote, or otherwise authorized the stable release. Stable cut is never autonomous.
---
...
2. Release build and full regression suite pass with the pinned SDK.
...
1. Create or continue a `codex/` task branch from current `origin/main`.
...
If the Forgejo route is available, mirror the immutable stable tag only after
the GitHub SHA matches:

```powershell
git push forgejo vX.Y.Z
```

Forgejo unavailability is reported; it is never repaired by launching a VPN on
...
`sign-android.yml` is only a legacy manual fallback.
