...
when: The user explicitly authorizes a rolling candidate release. Never tag, release, deploy, merge or cut stable autonomously.
---
...
2. Run `tools/verify-last-commit-ci.ps1`; any red or in-progress result is STOP.
3. Run the Release solution build, full tests, relevant visual tests and an
   independent review. Record every surviving finding in `plans/OPEN-DEFECTS.md`.
4. `tools/check-open-p0.ps1` must pass unless the owner explicitly records a
   waiver.
5. `AppVersion.Version` must exactly equal `X.Y.Z-rN`.
