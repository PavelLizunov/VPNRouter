...
```bash
powershell -ExecutionPolicy Bypass -File .claude/skills/post-ship-mcp-verify/scripts/post-ship-install-launch.ps1 -Version "2.X.Y-rN"
```
...
- **Not a stable-cut gate** — stable cut requires explicit user
  confirmation per `CLAUDE.md` rule #6. This skill's GREEN report is
...
- **Progressive disclosure**: SKILL.md (this file) under 500 lines holds
  ...
  `references/`. Claude reads only the relevant checklist for the
...
- `.claude/skills/ship-rolling-candidate/SKILL.md` — upstream skill that
  chains into this one
- `CLAUDE.md` rule #1a — "MCP test mandatory after every ship" (the
  policy this skill enforces)
- `CLAUDE.md` rule #11 — CI-gate after every push (Phase 1 underpinning)
