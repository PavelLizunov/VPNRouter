...
public sealed class UdpEchoProcessor
{
    ...
    private readonly Dictionary<string, long> _lastSequence = new(StringComparer.Ordinal);
    ...
    public UdpEchoDisposition Process(IPAddress source, ReadOnlySpan<byte> request, DateTimeOffset now, out byte[]? response)
    {
        ...
        if (_lastSequence.TryGetValue(key, out var last) && sequence <= last) return UdpEchoDisposition.Replay;
