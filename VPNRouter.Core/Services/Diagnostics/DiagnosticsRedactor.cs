...
public static class DiagnosticsRedactor
{
    ...
    private static readonly Regex _logKeyValueSecret = new(
        @"(?i)\b(password|passwd|pass|secret|token|uuid|short[_-]?id|private[_-]?key|api[_-]?key|psk|pre[_-]?shared[_-]?key|preshared[_-]?key|auth|authorization|proxy[-_]?authorization|credential|obfs[_-]?password|tg[_-]?proxy[_-]?secret)\b(\s*[=:]\s*)(""?)(?:(?:bearer|basic|token|digest|negotiate)\s+)?([^\s""',]+)",
