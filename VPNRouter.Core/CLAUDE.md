...
| `SplitTunnelDriverManager.cs` | **True-split W1.1-P2 (Windows)**. Sealed `ISplitTunnelDriver`: SCM idempotent create/adopt (никогда не стопаем службу) + collision-guard (чужой Mullvad не трогаем), один exclusive overlapped device-handle + `SemaphoreSlim` control-plane, engage/disengage (Disengage=RESET, inert), WFP sublayers, crash-sweep (`SweepStaleStateAsync`), `NetworkChange` re-register, **fail-open на каждом пути** (`EngageAsync` возвращает false, никогда не throw — excluded падают на post-capture `process_name→direct`). Event-pump — P3. Seam-паттерн = `IWindowsDnsHardening`. Не заинжекчен в VpnEngine до W1.2. |
