...
- **Encrypted DNS defaults** — DNS for VPN-routed apps is detoured through the encrypted proxy/tunnel; direct and smart public lookups use Cloudflare DoH, and RU bypass uses Yandex DoH. This safeguard does not replace explicitly configured custom DNS resolvers; if bootstrap DNS is missing, VPNRouter synthesizes literal-IP Cloudflare DoH. Only configured LAN suffixes may use the OS resolver.
