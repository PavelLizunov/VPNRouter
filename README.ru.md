...
- **Шифрованный DNS по умолчанию** — DNS выбранных приложений идёт через зашифрованный прокси/туннель; прямые и smart-запросы к публичному DNS используют Cloudflare DoH, а RU bypass — Yandex DoH. Эта защита не заменяет явно заданные DNS-серверы custom-конфига; если bootstrap DNS отсутствует, VPNRouter добавляет Cloudflare DoH с literal IP. Только явно настроенные LAN-суффиксы могут использовать системный resolver.
