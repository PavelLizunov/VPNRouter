...
### Решение владельца 2026-08-02

Закрыто без реализации. После DR-07 `System.Management` остаётся только в трёх
Windows-участках (`ProcessScanner`, `ProcessOwnership`,
`SplitTunnelDriverManager`). Ожидаемая экономия около 315 КБ не оправдывает
замену проверенной WMI-семантики новым Toolhelp/SCM interop и обязательный
повторный runtime-контур. Возвращаться к задаче только при реальном дефекте WMI
или существенно большей измеренной выгоде.
