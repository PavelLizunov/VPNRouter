...
- [ ] Release solution build: `dotnet build VPNRouter.sln -c Release` — zero errors.
- [ ] Focused parser/config tests pass.
- [ ] Full discovered test suite passes.
- [ ] No capability/no detour output remains unchanged.
- [ ] Missing upstream fails closed; no direct S5 fallback.
- [ ] Generated chain JSON passes bundled sing-box check.
- [ ] README/zone docs and this Outcome are current.
- [ ] Independent correctness, compatibility, simplicity, and security reviews have no unresolved important findings.
- [ ] PR CI is green; merge/release/WINBRAT installation waits for an explicit owner command.
