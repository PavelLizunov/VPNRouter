...
- [ ] **B. DNS-leak** browserleaks: только exit/vpn-dns, ноль RU-резолверов. ← сначала diagnostic (включить СУЩЕСТВУЮЩИЙ тумблер DnsLeakLockdown → browserleaks), потом код 1A по результату (см. Фаза-1 1A). ЖДУ результат тестера.
- [ ] **Не сломано:** Roblox + браузинг; VLESS/Hy2 без регрессий. ← браузинг на AWG у тестера работает (Dota+regions); Roblox отдельно не переспрошен.
