...
- [x] **A. Dota на AWG** — ✅ **ПОДТВЕРЖДЕНО ТЕСТЕРОМ 2026-07-02** («Dota исправилась, пинг отдаёт») на v2.45.0-r12 (Germany AWG). WSAENOBUFS-retry сработал живьём — корневой фикс верен.
- [x] **B. DNS-leak** — ✅ **ТЕСТЕР 2026-07-03: утечки НЕТ** («DNS-leak на hy2 я не увидел, теперь работало норм»). DNS-корректность подтверждена живьём. Нюанс: проверено на **Hy2**; исходный browserleaks-leak был на AWG-full-tunnel (SMHNR) — отдельно на AWG в этот раунд не переснимали, но тестер утечек не сообщает. 1A auto-arm НЕ понадобился (утечки нет) — остаётся как опциональный defense-in-depth, не блокер.
- [x] **Не сломано:** браузинг на AWG работает (Dota+regions), Hy2 DNS чистый; VLESS/Hy2 без регрессий (r13 resolver-fix). Roblox отдельно не переспрошен, регрессий не сообщено.

**ОБЕ живые приёмки закрыты (2026-07-03): Dota-на-AWG ✅ + DNS-leak ✅. Цель достигнута.**
Остаточные хвосты (НЕ блокеры цели, отдельные треды): ARC-на-настоящем-AWG ретест (r13 resolver-fix),
AWG/XHTTP на Mac (сейчас upstream, нужен darwin lx-build), косметический MTU-UX.

**Открылось при верификации (2026-07-02):** косметический MTU-UX-баг — поле «MTU TUN» показывает
generic `Tun.Mtu` (у тестера 1337), но для AWG конфиг ВСЕГДА берёт 1420 (r11 hardcode), игнорируя
поле. Реальный TUN=1420, поле вводит в заблуждение. Fix (кандидат в r13): либо показывать 1420
read-only при активном AWG (косметика), либо разрешить тюнинг AWG MTU с SDR-полом ≥1332 (для
зажатого underlay). — `ConfigGenerator.cs:1125` (hardcode) + `MainWindowViewModel.cs:2918/3650` (поле).

Dota ✅. Осталось: B (DNS-leak, ждём тестера) + косметический MTU-UX (r13). Stable-cut — по команде user'а.
