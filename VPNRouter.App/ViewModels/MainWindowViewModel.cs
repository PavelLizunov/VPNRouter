...
public partial class MainWindowViewModel : ViewModelBase
{
    ...
    public string LblDpiStrategies => IsRussian
        ? "Discord + YouTube + все сервисы (TCP + UDP):\n• general — multisplit + Discord voice UDP\n• general (ALT) — fake+fakedsplit, ts fooling\n• general (ALT2) — агрессивная (multidisorder, md5sig)\n• general (ALT3) — fake+multisplit, ts, x8 повторов\n\nТолько TCP (для VPN серверов):\n• multisplit — фрагменты (безопасно)\n• fake+multisplit — фейк + фрагменты\n• custom — свои аргументы"
        : "Discord + YouTube + all services (TCP + UDP):\n• general — multisplit + Discord voice UDP\n• general (ALT) — fake+fakedsplit, ts fooling\n• general (ALT2) — aggressive (multidisorder, md5sig)\n• general (ALT3) — fake+multisplit, ts, x8 repeats\n\nTCP only (for VPN servers):\n• multisplit — fragments (safest)\n• fake+multisplit — fake + fragments\n• custom — your own arguments";
