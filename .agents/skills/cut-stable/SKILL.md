...
Promotes `vX.Y.Z-rN` to stable `vX.Y.Z` (без суффикса). Per Codex.local.md
...
в `AGENTS.md`). Cut НЕ autonomous. User паузит явной командой "hold stable".
...
User confirmed <r-version> fixes work. Promoting per Codex.local.md
...
В `~/.Codex/projects/.../memory/MEMORY.md`:
