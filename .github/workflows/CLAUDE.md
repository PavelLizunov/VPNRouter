...
| `test.yml` (job `test`) | `push`/`PR` к `main`+`v3.0-prep` + `workflow_dispatch` | .NET 10 `dotnet test` VPNRouter.Tests (SDK from `global.json`) — primary regression gate. Без `paths:` фильтра. Filter исключает Headless/PageScreenshot/VisualDiff (Skia/display на ubuntu). Job id `test` — branch-protection required-check, не renameить. |
