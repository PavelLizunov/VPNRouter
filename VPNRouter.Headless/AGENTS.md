...
7. **Runtime Policy & Fail-Closed Readiness**: Linux Headless operates under `SingBoxRuntimePolicy`. Default production policy is strictly unavailable (`untrusted`) because no trusted artifact is yet authorized. Mere file existence on disk at `AppPaths.SingBoxExePath` does not establish readiness, integrity, or authority to launch code. Startup copies from bundle, cronet colocation, and pkexec/sudo elevation fallbacks are forbidden under restricted policy.
